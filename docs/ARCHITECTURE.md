...
- **Detect** — a rule-based risk engine scores sprint health (SCRUM-9) and flags the
  single hidden at-risk story (SCRUM-10). Owner emphasis: **analytics**. *Built*
  (branch `feat/detect-engine`, off `feat/foundation-ingest`).
...
| Risk engine | `app/risk/*`, `app/models/risk.py`, `app/api/risk.py` | Sprint-health score + hidden-risk detection | built (SCRUM-9, SCRUM-10) |
...
**Implementation notes (SCRUM-9/SCRUM-10, built):**
- `score_sprint_health` and `RiskDetector.detect` are pure functions of `session` +
  `now` — no implicit clock, no writes from the health path. The `now` default lives
  one layer up, at the API (`app/api/risk.py`, `app/risk/config.py::DEMO_NOW`), so the
  engine functions stay fully explicit/testable and a real caller (SCRUM-18) can pass
  `datetime.now(UTC)`.
- `StalledCriticalStoryDetector` persists findings via `app/risk/persistence.py` into
  `app/models/risk.py::Risk`, upserting on the unique `(project_id, risk_type,
  target_external_id)` key so re-detect is idempotent.
- **Known gap carried forward:** `meta.sprint{id,name,state}` (pinned above) is not yet
  populated by `JiraConnector` — see `TODO(SCRUM-7)` in `app/risk/sprint_health.py`.
  Until it lands, sprint-scoping falls back to "all issue signals in the project,"
  which is correct only for the current single-sprint MVP scope.
- **Audit seam:** detection calls `app/risk/audit.py::AuditSink.record_detection`
  (default `NullAuditSink`, a no-op) instead of importing `app/govern/audit.py`
  directly, so Detect does not hard-depend on SCRUM-13's landing order. Wire the real
  `AuditService` in at the SCRUM-13 handoff — see that module's `TODO`.
- PR↔issue correlation (for the starved-PR check and the last-commit citation) uses
  the issue key embedded in the PR title / `meta.head_ref`, since GitHub commit
  signals do not yet carry `meta.issue_keys` either (same SCRUM-7 gap, noted in
  `app/risk/rules.py`).
...
| `GET /projects/{id}/sprints/{sprint_id}/health` | built (SCRUM-9) | Sprint health + burndown |
| `GET /projects/{id}/risks`, `POST .../risks/detect` | built (SCRUM-10) | Risk findings |
